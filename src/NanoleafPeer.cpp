    ...
    try
    {
        ...
        if(requestNewApiKey)
        {
            ...
            if(http.getContentSize() == 0)
            {
                ...
                _bl->globalServiceMessages.set(NANOLEAF_FAMILY_ID, "", 0, std::to_string(_peerID), BaseLib::ServiceMessagePriority::kError, BaseLib::HelperFunctions::getTimeSeconds(), "l10n.nanoleaf.pressPowerButton", std::list<std::string>{ std::to_string(_peerID), getName(), _ip }, data, 1);
